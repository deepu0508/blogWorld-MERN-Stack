...
export default function ReadBlog(props) {
    ...
    (async () => {
      try {
        const response = await fetch(
          `${host}/api/blogInfo/infoFetch/${blogInfoId}`,
          {
            method: "POST",
            headers: {
  ...
  const toggleComment = (id) => {
    ...
    if (id === "readComment") {
      ...
      if (closeAll.classList.contains("show")) {
  ...
  const fetchallCmt = async () => {
    try {
      const responseCmt = await fetch(`${host}/api/comments/fetchall/${_id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "bw_auth_token": sessionStorage.getItem("bw_auth_token"),
  ...
  const deleteBlog = async () => {
    try {
      const response = await fetch(`${host}/api/blog/deleteblog/${_id}`, {
        ...
        headers: {
          "Content-Type": "application/json",
          "auth-token": sessionStorage.getItem("auth-token"), // ###########---> auth token send backend for verify login user
          "bw_auth_token": sessionStorage.getItem("bw_auth_token"), // ###########---> bw auth token send backend for verify valid user
        },
      });
      const data = await response.json();
      // console.log(data);
    ...
    <>
      <div className="df flex-column bg-dark text-dark mainContainer">
        <div className="container df flex-column rounded my-3 p-3 blogPage">
          <div className="container df flex-wrap-reverse rounded p-1 align-items-end">
            <div className="container-sm rounded image-part-blog">
              <img
                ...
                alt=""
                className="rounded img-thumbnail img-show-blog"
              />
            </div>
          ...
          {file && (
            <div className="container rounded file-part">
              <Link exact to={`http://localhost:8900/${file}`} className="m-2">
                Blog File(click me...)
              </Link>
            </div>
          )}
          ...
          <div className="container df p-3 my-3 rounded justify-content-between footer-blog">
            <div className="df like-comment-part">
              ...
              <div className="d-flex flex-row mx-2 likes-part">
                ...
                />
                <p className="numLikes" id="numLikes1">
                  {like.length - 1}
                </p>
              </div>
              ...
              <div className="d-flex flex-row comments-part">
                <img
                  ...
                  onClick={() => {
                    moveCom();
                    toggleComment("addComment");
                  }}
                />
                <div className="numLikes numComments" id="numComments">
                  {allComment.length}
                </div>
              </div>
                      ...
                      <div className="card-body">
                        {allComment.length !== 0 ? (
                          allComment.map((cmt) => {
                            return (
                              <li
                                className="list-group-item d-flex justify-content-between align-items-start my-2"
                                key={cmt._id}
                              >
